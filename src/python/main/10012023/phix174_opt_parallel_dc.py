...
def run_pt(sim, gen, ID, pA, pB, pD, tJ, tF, tG, tH, base_dir, date):
    ...
    genomic_coords = pd.read_csv(base_dir + "data/decompressed_genomic_coords_2.csv")
...
def get_error(sim, gen, trials, base_dir, date):
    ...
    if (gen == 0):
        if (len(results) != 11):
            ...
            # fit to protein abundances
            results["exp"] = [6698, 5734, 711, 575, 12403, 10480, 6826, 2875, 538, 10219, 605]
            error = round(mean_squared_error(results.exp, results.protein, squared=False), 5)
            return (error)
