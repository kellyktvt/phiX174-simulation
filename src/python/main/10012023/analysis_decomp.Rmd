...
date <- "10012023"
filedir <- paste("src/python/main/", date, "/output/", sep="")
...
plot_reports <- function(filedir,  title){
  ...
  sim1_file <- paste0(filedir, "sim_1_report.csv")
  if (!file.exists(sim1_file)) {
    stop(paste("Error: File", sim1_file, "does not exist in current working directory."))
  }
  
  #figure <- get_reports(filedir) %>% 
  figure <- read_csv(sim1_file) %>%
    # filter(!is.na(gen)) %>% 
    filter(gen > 0) %>% 
    filter(error < 10000) %>% 
    ggplot(aes(x = gen)) +
